...
def main():
    ...
    for elem in range(len(array_random)):
        min = elem # index of start min element
        for j in range(elem + 1, len(array_random)):
        ...
        array_random[min], array_random[elem] = array_random[elem], array_random[min]
    ...
    even_array = [i for i in array_random if not i % 2 ]
